...
class ChatOperatorsMessagesBloc extends Bloc<ChatOperatorsMessagesEvent, ChatOperatorsMessagesState>{
  ...
  ChatOperatorsMessagesBloc({this.serverRepository}) : super(ChatOperatorsMessagesInitial()){
  ...
  Future<void> _onFetchOperatorsChatMessages(FetchOperatorsChatMessages event,Emitter<ChatOperatorsMessagesState> emit) async {
    ...
    if( state is ChatOperatorsMessagesInitial){
      emit(ChatOperatorsMessagesLoaded(server:
      event.server,
          chat: event.chat,
          messages: msgs,
  ...
  Future<void> _onPostOperatorsMessage(PostOperatorsMessage event,Emitter<ChatOperatorsMessagesState> emit) async {
    final currentState = state;
    if (currentState is ChatOperatorsMessagesLoaded) {
      var id=await serverRepository?.postOperatorsMesssage(event.server, event.chat, event.message!);
      add(FetchOperatorsChatMessages(server: event.server, chat: event.chat));
    }
  }

  Future<void> _onCloseOperatorsChat(CloseOperatorsChat event,Emitter<ChatOperatorsMessagesState> emit) async {
    bool? closed = false;
    final currentState = state;
    if (currentState is ChatOperatorsMessagesLoaded) {
  ...
  Future<void> _onDeleteOperatorsChat(DeleteOperatorsChat event,Emitter<ChatOperatorsMessagesState> emit) async {
    bool? closed = false;
    final currentState = state;
    if (currentState is ChatOperatorsMessagesLoaded) {
      emit(currentState.copyWith(isLoading: true));
